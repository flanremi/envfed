...
class TrainingHelper:
    ...
    # 不要执行这个
    def __val_once(self, hyp, device, callbacks):
        opt = self.opt
        ...
        return results, maps, _


    # 不要执行这个
    def __train(self, hyp, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
        opt = self.opt
        save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg, resume, noval, nosave, workers, freeze = \
            Path(opt.save_dir), opt.epochs, opt.batch_size, opt.weights, opt.single_cls, opt.evolve, opt.data, opt.cfg, \
            opt.resume, opt.noval, opt.nosave, opt.workers, opt.freeze
            ...
            if RANK in {-1, 0}:
                ...
                if not noval or final_epoch:  # Calculate mAP
                    results, maps, _ = validate.run(data_dict,
                                                    ...
                                                    callbacks=callbacks,
                                                    compute_loss=compute_loss)
                    tmp_val_result.append({"epoch":epoch, "results":results, "maps":maps.tolist(), "_":_})
                # Update best mAP
        ...
        with open(w / "val.txt", "w+") as _file:
            _file.write(json.dumps(tmp_val_result))
        return results
    ...
    def main(self, callbacks=Callbacks()):
        ...
        # Train
        if not opt.evolve:
            self.__train(opt.hyp, device, callbacks)
        ...
        # Evolve hyperparameters (optional)
            ...
            for _ in range(opt.evolve):  # generations to evolve
                ...
                # Train mutation
                results = self.__train(hyp.copy(), device, callbacks)
                callbacks = Callbacks()
    ...
    def model_val(self, model, callbacks=Callbacks()):
        ema = ModelEMA(model) if RANK in {-1, 0} else None
        amp = check_amp(model)  # check AMP
        compute_loss = ComputeLoss(model)  # init loss class
        opt = self.opt
        ...
        if self.val_loader is None:
            ...
            if RANK in {-1, 0}:
                ...
                # Process custom dataset artifact link
                self.data_dict = loggers.remote_dataset
                if resume:  # If resuming runs from remote artifact
                    weights, epochs, hyp, batch_size = opt.weights, opt.epochs, opt.hyp, opt.batch_size
            ...
            init_seeds(opt.seed + 1 + RANK, deterministic=True)
            with torch_distributed_zero_first(LOCAL_RANK):
                self.data_dict = self.data_dict or check_dataset(data)  # check if None
            val_path = self.data_dict['val']
            nc = 1 if single_cls else int(self.data_dict['nc'])  # number of classes
            names = {0: 'item'} if single_cls and len(self.data_dict['names']) != 1 else self.data_dict['names']  # class names
            is_coco = isinstance(val_path, str) and val_path.endswith('coco/val2017.txt')  # COCO dataset
            ...
            # Image size
            gs = max(int(model.stride.max()), 32)  # grid size (max stride)
            self.imgsz = check_img_size(opt.imgsz, gs, floor=gs * 2)  # verify imgsz is gs-multiple
            ...
            if cuda and RANK == -1 and torch.cuda.device_count() > 1:
                LOGGER.warning(
                    'WARNING ⚠️ DP not recommended, use torch.distributed.run for best DDP Multi-GPU results.\n'
                    'See Multi-GPU Tutorial at https://docs.ultralytics.com/yolov5/tut'
                    'orials/multi_gpu_training to get started.'
                )
                model = torch.nn.DataParallel(model)
    ...
    # 仅获取val
    def main_val(self, callbacks=Callbacks()):
        opt = self.opt
        opt.nosave = True
        opt.epochs = 1
        # Checks
        ...
        return self.__val_once(opt.hyp, device, callbacks)
...
def sum_model(old_model, new_model, step):
    tmp_dict = {}
    for key, var in old_model.state_dict().items():
        old = var.clone()
        new = new_model.state_dict()[key].clone()
        _r = old * ((step + 1) /( step + 2)) + new * (1 / (step + 2))
        tmp_dict.update({key: _r})
    old_model.load_state_dict(tmp_dict)
    return old_model


def getParamlistByModel(model_url):
    model = torch.load(model_url)
    model = model['model'].state_dict().items()
    tmp = []
