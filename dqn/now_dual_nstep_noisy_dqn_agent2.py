...
class Qnet(torch.nn.Module):
    ...
    def __init__(self, device, input_dim, action_dim, ):
        super().__init__()
        self.device = device
        hidden = 2 ** (int(math.log2(input_dim)) + 5)
...
class DDQN:
    ...
    def save_net(self):
        global current_path
        torch.save(self.q_net.state_dict(), current_path + "net/" + self.name + "dual_nstep_noisy_dqn_q_net2")
        torch.save(self.target_q_net.state_dict(),
                   current_path + "net/" + self.name + "dual_nstep_noisy_dqn_target_q_net2")

    def load_net(self):
        global current_path
        self.q_net.load_state_dict(torch.load(current_path + "net/" + self.name + "dual_nstep_noisy_dqn_q_net2"))
        self.target_q_net.load_state_dict(torch.load(current_path + "net/" + self.name +
                                                     "dual_nstep_noisy_dqn_target_q_net2"))
        print("n模型加载成功")
    ...
    def take_action(self, state, explore=True):  # epsilon-贪婪策略采取动作
        if explore:
            if np.random.random() < self.epsilon:
                ...
                tmp = self.q_net(state)
                values, action = torch.topk(tmp, 5)
    ...
    def take_action2(self, state):  # 无贪婪
        state = torch.tensor(state, dtype=torch.float).to(self.device)
        tmp = self.q_net(state)
        values, action = torch.topk(tmp, 3)
        return action.tolist()
    ...
    def update(self, n_sample):
        ...
        def get_loss(transition_dict):
            ...
            actions = torch.tensor(transition_dict[1]).to(
                self.device)
            ...
            q_values = C.sum(dim=1, keepdim=True)
            ...
            value, indices = actiont.topk(3, dim=1)
            target_q_values = self.target_q_net(next_states)
            batch_size = target_q_values.size(0)
            row_indices = torch.arange(batch_size).unsqueeze(-1).expand_as(indices)
            selected_values = target_q_values[row_indices, indices]
            max_next_q_values = selected_values.sum(dim=1).unsqueeze(1)

            q_targets = rewards + (self.gamma ** steps) * max_next_q_values * (1 - dones)  # TD误差目标
